...
function getPieceList (files, pieceLength, cb, opts) {
  cb = once(cb)
  const pieces = []
  let length = 0
  const totalSize = opts.onProgress && files.reduce(sumLength, 0)
  ...
  function onData (chunk) {
    ...
    remainingHashes += 1
    pieceNum += 1
    opts.onProgress && opts.onProgress(totalSize, length)
  }
